...
async def get_top_competitors(keyword: str):
    logger.info("[competitor.py] Starting get_top_competitors function for keyword: %s", keyword)
    ...
    try:
        logger.info("[competitor.py] Stage: Making HTTP request to DuckDuckGo for competitors")
        async with httpx.AsyncClient(timeout=COMPETITOR_FETCH_TIMEOUT_SECONDS) as client:
            response = await client.get(search_url, headers=headers)
            response.raise_for_status()
        logger.info("[competitor.py] Stage: HTTP request completed, parsing HTML")
    except Exception:
        return []
    ...
    logger.info("[competitor.py] Found %d competitor links", len(links))
    return links[:3]
