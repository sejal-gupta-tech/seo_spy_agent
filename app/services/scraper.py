...
def _favicon_status_label(crawl_data: dict) -> str:
    ...
    favicon_resource = crawl_data.get("site_favicon", {})
    if favicon_resource.get("status") == "Present":
        return f"Found (/favicon.ico · 200)"
...
class SEOAuditPipeline:
    ...
    async def _crawl_stage(self):
        ...
        logger.info("[scraper.py] Stage: Starting crawl stage")
        logger.info("Starting crawl stage for %s", self.url)
        await self.emit({
    ...
    async def _audit_stage(self):
        ...
        logger.info("[scraper.py] Stage: Starting audit stage")
        await self.emit({
        ...
        logger.info(
            "Audit stage completed internal metrics for %s: page_audits=%s, overall_score=%s",
            self.url,
            len(self.page_audits),
            self.sitewide_audit.get("overall_score", "N/A"),
        )
    ...
    async def _ai_strategy_stage(self):
        ...
        logger.info("[scraper.py] Stage: Starting AI strategy stage")
        logger.info("Starting AI strategy stage for %s", self.url)
        ...
        logger.info("[scraper.py] AI API call: Starting generate_consolidated_strategy")
        ...
        logger.info("[scraper.py] AI API call: Completed generate_consolidated_strategy")
    ...
    async def _competition_stage(self):
        ...
        logger.info("[scraper.py] Stage: Starting competition stage")
        await self.emit({
        ...
        logger.info(
            "Competition stage completed for %s: competitors=%s, keyword_overlap=%s",
            self.url,
            len(self.competitors),
            self.comparison_result.get("keyword_overlap_score", "N/A"),
        )

    async def _report_stage(self):
        """Phase 5: Report Generation."""
        logger.info("[scraper.py] Stage: Starting report stage")
        logger.info("Starting report stage for %s", self.url)
        await self.emit({
    ...
    async def execute(self) -> dict:
        ...
        logger.info("[scraper.py] Starting execute function for SEO audit pipeline")
        logger.info(f"Starting SEO audit pipeline for: {self.url}")
        try:
            await self.emit({
                "type": "stage", "stage": "crawl", "status": "active",
            ...
            logger.info(f"Emitted initial progress event for: {self.url}")
            await self._crawl_stage()
            logger.info(f"Crawl stage completed for: {self.url} with {len(self.pages)} pages")
            await self._audit_stage()
            logger.info(f"Audit stage completed for: {self.url} with overall score {self.sitewide_audit.get('overall_score', 'N/A')}")
            await self._ai_strategy_stage()
            logger.info(f"AI strategy stage completed for: {self.url} with main keyword '{self.ai_result.get('keywords', ['N/A'])[0] if self.ai_result.get('keywords') else 'N/A'}'")
            await self._competition_stage()
            logger.info(f"Competition stage completed for: {self.url} with keyword overlap score {self.comparison_result.get('keyword_overlap_score', 'N/A')}")
            await self._report_stage()
            logger.info(f"Report generation completed for: {self.url} with report ID {self.final_report_id}")
            logger.info(f"SEO audit pipeline completed successfully for: {self.url} in {round(time.perf_counter() - self.start_time, 2)} seconds")
            logger.info(f"Final assembled result for {self.url}: {self._assemble_final_result()}")
            logger.info(f"Emitting final progress event for: {self.url}")
            logger.info(f"Final report URL: /download-report/{self.final_report_id}")
            logger.info(f"Final executive summary for {self.url}: {self.executive_summary}")
            logger.info(f"Final management summary for {self.url}: {self.management_summary}")
            logger.info(f"Final page speed data for {self.url}: {self.page_speed_data}")
            logger.info(f"Final link analysis for {self.url}: {self.link_analysis}")
            logger.info(f"Final AI insights for {self.url}: {self.ai_insights}")
