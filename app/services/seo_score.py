...
class SEOScoringEngine:
    ...
    def calculate_technical_score(self, page_data: dict) -> float:
        ...
        score = (t_https + t_mobile + t_broken) / 3
        logger.debug(
            "Technical score calculation for %s: https=%s mobile=%s broken=%s score=%s",
            url,
            t_https,
            t_mobile,
            t_broken,
    ...
    def calculate_onpage_score(self, title: str, meta_desc: str, h1_count: int) -> float:
        ...
        score = (o_title + o_meta + o_h1) / 3
        logger.debug(
            "On-page score calculation: title_len=%s meta_desc_present=%s h1_count=%s score=%s",
            len(title),
            o_meta == 100,
            h1_count,
            score,
    ...
    def calculate_health(self, page_data: dict) -> dict:
        ...
        try:
            ...
            logger.info(
                "Calculating SEO health for URL: %s",
            ...
            logger.info(
                "SEO health calculated for %s: final_score=%s scores=%s",
                page_data.get("url", "unknown"),
                final_score,
                {
                    "technical": int(tech),
