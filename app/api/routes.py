...
async def fix_issue(data: FixRequest):
    logger.info("[routes.py] Generate fix endpoint called")
    try:
...
async def analyze(data: URLRequest):
    logger.info("[routes.py] Analyze endpoint called for URL: %s", data.url)
    normalized_url = normalize_url(data.url)
    logger.info("Received analysis request for URL: %s", normalized_url)

    if not is_valid_url(normalized_url):
        logger.warning("Invalid URL received for analysis: %s", data.url)
        raise HTTPException(status_code=400, detail="Invalid URL.")

    try:
        result = await analyze_url(normalized_url)

        logger.info("Analysis completed for URL: %s", normalized_url)
...
async def submit_analysis_job(data: URLRequest):
    logger.info("[routes.py] Analysis jobs endpoint called for URL: %s", data.url)
    normalized_url = normalize_url(data.url)
    logger.info("Received background analysis job request for URL: %s", normalized_url)

    if not is_valid_url(normalized_url):
        logger.warning("Invalid URL received for background job: %s", data.url)
        raise HTTPException(status_code=400, detail="Invalid URL")

    job = await create_analysis_job(normalized_url)
    logger.info("Background analysis job created for URL: %s, job_id=%s", normalized_url, job.job_id if hasattr(job, 'job_id') else 'unknown')
    return job
...
async def analyze_stream(data: URLRequest):
    logger.info("[routes.py] Stream analysis endpoint called for URL: %s", data.url)
    normalized_url = normalize_url(data.url)
    logger.info("Received streaming analysis request for URL: %s", normalized_url)

    if not is_valid_url(normalized_url):
        logger.warning("Invalid URL received for stream analysis: %s", data.url)
        raise HTTPException(status_code=400, detail="Invalid URL.")
...
def download_report(task_id: str = Depends(_validate_task_id)):
    ...
    """
    logger.info("[routes.py] Download report endpoint called for task_id: %s", task_id)
    logger.info("Download report requested for task_id: %s", task_id)
...
async def list_projects():
    logger.info("[routes.py] List projects endpoint called")
    return await get_all_projects()


@router.get("/projects/{project_id}", dependencies=[Depends(_require_api_key)])
async def get_project(project_id: str):
    logger.info("[routes.py] Get project endpoint called for project_id: %s", project_id)
